...
def get_zne_data (st, inv, starttime):
    if len(st) != 3:
        sys.exit("The length of st must be 3.")
...
locactEQOPT = config["locactEQOPT"]
...
bv1OPT = config["bv1OPT"]
bv2OPT = config["bv2OPT"]
...
Rvel = config["Rvel"]
tw_pre_tw = config["tw_pre_tw"]
tw_trim = config["tw_trim"]

if locactEQOPT:
    tw_pre_tw = -5 # sec before p wave arrival
    tw_trim =  25 # 20-s window length

fl2 = config["fl2"]
fl = config["fl"]
fh = config["fh"]
fh2 = config["fh2"]
logOPT = config["logOPT"]
